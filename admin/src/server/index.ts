  ...
  transformRequestData: (
    ...
    switch (code) {
      ...
      case ResultEnum.TIMEOUT:
        errorMsg = "登录超时，请重新登录!";
        console.log("登录身份已失效，请重新登录!");
        break;
    }

    if (!code || !result || !message) {
      if (!code) {
        errorMessage += " code:为空";
      }
      if (!result) {
        errorMessage += " result:为空";
      }
      if (!message) {
        errorMessage += " message:为空";
      }
      errorMessage += "。请检查前后端约定格式";
    ...
    deepMerge(
      {
        ...
        requestOptions: {
          ...
          // 接口拼接地址
          urlPrefix: import.meta.env.VITE_BASE_URL_API,
          //  是否加入时间戳
          joinTime: true,
          // 忽略重复请求
